...
use mdbook::config::{Config, HtmlConfig, Playpen};
use mdbook::errors::*;
use mdbook::renderer::{RenderContext, Renderer};
use mdbook::theme::{self, playpen_editor, Theme};
use mdbook::utils;
...
use crate::post_processor::{
    CodeBlockProcessor, HeaderLinkProcessor, PlaypenProcessor, PostProcessor,
};
//
...
impl Default for HtmlHandlebars {
    fn default() -> Self {
        Self {
            name: "althtml".to_owned(),
            /// User-define post_processors
            post_processors: Vec::<Box<dyn PostProcessor>>::new(),
        }
    }
}
...
impl HtmlHandlebars {
    ...
    pub fn change_output_dir(&mut self, new_dir: &str) {
        self.name = new_dir.to_owned();
    }

    pub fn add_post_processor<P: PostProcessor + 'static>(&mut self, renderer: P) {
        self.post_processors.push(Box::new(renderer));
    }
    ...
    fn post_process(&self, rendered: String, playpen_config: &Playpen) -> String {
        ...
        let hlp = HeaderLinkProcessor::new();
        let cbp = CodeBlockProcessor::new();
        let ppp = PlaypenProcessor::new_with_config(playpen_config);
        let builtin_post_processors: Vec<Box<dyn PostProcessor>> =
...
impl Renderer for HtmlHandlebars {
    ...
    fn render(&self, ctx: &RenderContext) -> mdbook::errors::Result<()> {
        ...
        // Copy all remaining files except for markdown files
...
// todo: move to popost_processor.rs
//#[cfg(test)]
//mod tests {
//    use super::*;
//
